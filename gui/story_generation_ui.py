...
#     print("generate story is called ")
#     print(prompt)
#     print(genre)
...
#         chat = ChatOpenAI(temperature=0.9 , model="gpt-3.5-turbo-16k", client=any , openai_api_key="")
...
def generate_story(prompt, genre, number_of_words):
    ...
    try:
        ...
        chat = ChatOpenAI(temperature=0.9, model="gpt-3.5-turbo-16k", openai_api_key="")
        ...
        messages = [
            ...
            HumanMessage(content=f""" Write a story with the following plot : {prompt} with the following genre : {genre} The story should be {number_of_words} words long. Instead of pronouns, use the names of the characters and dont use apostrophe symbols"""),
        ]
        
        try:
            result = chat.generate([messages])
            story = result.generations[0][0].text
            length_of_ai_story = len(story.split(" "))
            word_list = story.split(".")
            messages2 = [
            # SystemMessage(content="In a world of boundless creativity, you are an AI storyteller who weaves intriguing tales with depth. Join us on a journey where words come alive, characters thrive, and stories leave an everlasting impact. Let's create something magical together."),
            HumanMessage(content=f"""correct this {word_list} according to dalle so each sentence of the array is going to be used to create an image and return as an list to be stored in json format , return only the list and nothing else , and the list shpuld contain only the sentences , dont give any explanations"""),
        ]

            result2 = chat.generate([messages2])
            word_list = result2.generations[0][0].text
        ...
        print(f"AI story is {length_of_ai_story} words long")
        print(story)
        ...
        try:
            ...
            if current_selected_project is None:
                ...
                if os.path.exists(json_file_path):
                    ...
                    # Update the existing data with the new values
                    existing_data["content"] = str(story)
                    existing_data["word_lists"]=json.dumps(word_list)
                    with open(json_file_path, "w") as json_file:
                        json.dump(existing_data, json_file)
        except Exception as e:
            raise gr.Error("Please select a project first")
...
def Set_story(story):
    ...
    try:
        try:    
            ...
            word_list = [' '.join(words[i:i+15]) for i in range(0, len(words), 15)]
            print(word_list)
            print("projnameg " ,get_project_name())
        except Exception as e:
            raise gr.Error("Something went wrong while generating the story. Please try again later.") 
...
def story_generation_ui(UI: gr.Blocks):
    print(1)
    print(1)
    print(1)
    with gr.Tab("Generate Story") as  story_generation:
        ...
        with gr.Row(visible=True):
            with gr.Column(visible=True):
                ...
                # generate_story_button.click( inputs=[prompt, genre, duration] , outputs=[generation_error]).success(lambda : gr.update(visible=True) , outputs=[edit_generation_column]).success(generate_story, inputs=[prompt, genre, number_of_words] , outputs=[edit_story])
                print(2)
                print(2)
                print(2)
                print(number_of_words.value)
                print(number_of_words)
                print(prompt.value)
                print(genre.value)
                print(number_of_words.value)
                print(number_of_words)
                generate_story_button.click(lambda: generate_story("A boy dreaming about rockets", "Children books", "150"))
...
#                     generate_story_button = gr.Button("Generate Story", size="sm", interactive=True, visible=True) 
#                     generate_story_button.click(lambda: print(f"Prompt: {prompt.value}, Genre: {genre.value}, Number of words: {number_of_words.value}"))
...
if __name__ == "__main__":
    generate_story("batman vs superman","superhero",150)